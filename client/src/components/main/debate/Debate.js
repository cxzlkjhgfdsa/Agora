...
function Debate({ visibleCounts, roomInfo, type }) {
  const [debateRoom, setDebateRoom] = useRecoilState(debateRoomsSelectorFamily(roomInfo.room_id));
  const resetDebateRoom = useResetRecoilState(debateRoomsSelectorFamily(roomInfo.user_id));
  
  const roomId = roomInfo.room_id;

  const title = roomInfo.room_name;

  const leftOpinion = roomInfo.room_opinion_left;
  const rightOpinion = roomInfo.room_opinion_right;

  const viewers = roomInfo.room_watch_cnt;
  const phases = roomInfo.room_phase;
  const minutes = roomInfo.room_phase_current_time_minute;
  const seconds = roomInfo.room_phase_current_time_second;

  const imageUrl = roomInfo.room_thumbnail_url 
  ? roomInfo.room_thumbnail_url 
  : NoImageAvailable;
  ...
  }, [resetDebateRoom, setDebateRoom])


  return (
    <Wrapper visibleCounts={visibleCounts} ref={locRef}>
            <ThumbnailInfoWrapper
            onclick={`location.href="/debate/room/${roomId}"`}
            ref={hoverRef}
            isHovered={isHovered}
            zIndex={zIndex}
            onMouseEnter={expand}
            ...
            >
                ...
                {isHovered
                    ? <RoomInfo content={roomInfo} isHovered={isHovered} ref={roomInfoRef} />
                    : null}
